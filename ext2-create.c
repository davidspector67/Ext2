...
#define get_bitmap(bitmap, last_reserved, num_free)																\
	int reserved_bytes = last_reserved / 8;														\
	for (int i = 0; i < reserved_bytes; i++) {													\
		bitmap[i] = ~bitmap[i];																	\
	}																							\
																								\
	int remaining_reserved_bits = last_reserved % 8;												\
	bitmap[reserved_bytes] = ~((~0) << remaining_reserved_bits);								\
																								\
	if (num_free < (8 - remaining_reserved_bits)) {										\
		bitmap[reserved_bytes] |= ~((~0) << (8 - remaining_reserved_bits - num_free + 1));	\
	}																							\
	int free_bits = num_free - (8 - remaining_reserved_bits);						\
	int free_bytes = free_bits / 8;															\
	int remaining_free_bits = free_bits % 8;												\
	int cur_index = reserved_bytes + free_bytes + 1;										\
	if (cur_index < BLOCK_SIZE) {															\
		bitmap[cur_index] = (~0) << remaining_free_bits;									\
		for (int i = cur_index+1; i < BLOCK_SIZE; i++) {									\
			bitmap[i] = ~bitmap[i];															\
		}																					\
	}																						\
...
void write_block_bitmap(int fd) {
	...
	get_bitmap(bitmap, LAST_BLOCK, NUM_FREE_BLOCKS);

	int status = write(fd, bitmap, sizeof(u32)*256);
	if (status == -1) {
		errno_exit("write");
	}
...
void write_inode_bitmap(int fd) {
	...
	char bitmap[BLOCK_SIZE] = {0};
	get_bitmap(bitmap, LAST_INO, NUM_FREE_INODES);
...
void write_inode_table(int fd) {
	...
	hello_world_inode.i_block[0] = HELLO_WORLD_FILE_BLOCKNO; // i_block is used to find address of the real data block, where it is stored
...
void write_root_dir_block(int fd) {
	...
	dir_entry_set(current_entry, EXT2_ROOT_INO, "."); // 2nd arg is inode loc; rrd is the name
